Although the Thompson farm employed a five crop rotation, our analysis focuses on corn and soybeans since the Boone farm data only includes information on those two crops.  After averaging expenses by year, and over the number of fields, we plot the difference in expenses between the two farms (by crop) after selecting differences that are substantively different from zero.  
...
The second biggest cost difference is for herbicide.  This expense is stable, hovering around $25/acre more for Boone compared to Thompson.  These differences are likely due to Thompson's organic farming practices.

There are also expenses that only Boone accrues.


```{r,warning=FALSE,message=FALSE,echo=FALSE}
boone_extra_expense_corn <- dat[complete.cases(dat),] %>% 
  filter(item_type == "Expense") %>%
  filter(farmer=="Boone") %>%
  filter(crop %in% c("Corn")) %>%
...
boone_extra_expense_corn

```

```{r}
boone_extra_expense_sb<-dat[complete.cases(dat),] %>% 
  filter(item_type == "Expense") %>%
  filter(farmer=="Boone") %>%
  filter(crop %in% c("SB")) %>%
...
Spring and fall tillage also added approximately $10/acre to the Boone farm.  As part of his organic farming, Thompson does not till his land.

Although Thompson has lower expenses than Boone, it could be a moot point if his yield is substantially lower than Boone.  Therefore, we will also compare yields to see if Thompson is able to still produce despite spending less on fertilizer and herbicide.  From looking at time series plots of average yields, it appears that Thompson is able to consistently produce corn and soybeans at an equal or slightly higher level than the Boone farm.    
...
We are interested in the expenses incurred from sequential planting of crops in rotation. Due to the nature of the data we have available, this exploration will be limited to the Thompson data set only, seeing as the Boone average data does not have rotation of crops.
