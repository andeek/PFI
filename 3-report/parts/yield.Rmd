...
plot(cornThompson$year,cornThompson$averageYield,type="l",main="Thompson Yield vs Boone Average-Corn",ylab="Yield",xlab="Year")
...
plot(soyThompson$year,soyThompson$averageYield,type="l",main="Thompson Yield vs Boone Average-Soybeans",ylim=c(20,70),ylab="Yield",xlab="Year")
...
plot(hayThompson$year,hayThompson$averageYield,type="l",col="black",ylim=c(0,9),main="Thompson Yield vs Boone Average-Hay",ylab="Yield",xlab="Year")
...
plot(oatsThompson$year,oatsThompson$averageYield,type="l",,ylim=c(30,150),main="Thompson Yield vs Boone Average-Oats",ylab="Yield",xlab="Year")
...
#print(mean(ttestCorn$value-ttestCorn$Value))
...
#print(mean(ttestsb$value-ttestsb$Value))
...
#print(mean(hayThompson$averageYield-hayCounty$Value))
...
#print(mean(ttestOats$value-ttestOats$Value))
...
The t-test statistics for corn, soybeans, hay, and oats respectively are 4.817, 8.729, 19.371, and 5.46 with corresponding p-values all approximately zero.  This leads to the conclusion that Thompson's yields are all significantly better than the Boone County average.  

The average yield differences are as follows.  Corn on Thompson's land produces 12.67 bushels per acre more than the average Boone farm.  Soybeans on Thompson's land produces 10.22 bushels per acre more than the average Boone farm. Hay on Thompson's land produces 2.79  dry tons per acre more than the average Boone farm. Oats on Thompson's land produces 22.91 bushels per acre more than the average Boone farm. 

## Yield by previous crop

In this section we will see if the crop rotation used by Thompson has any affect on yield.  Specifically we will look at how crops preform based on the crop planted the previous year.  Soybeans and oats are always preceded by corn according to Thompson's rotation system. However, corn is preceded by either hay or soybeans, and hay is preceded by either hay or oats. The boxplots below illustrate how the yield for corn and hay vary based on the crop that was planted on the same field during the previous year.
...
After examining how each individual expense related to the yield of each crop, we found three interesting relationships having to do with corn.
...
The plot above shows the expenses of ```Rotary_Hoe``` against the yield of corn with a linear trend fitted to the data. The trend suggests that the more money spent on ```Rotary_Hoe```, the higher the expected yield will be.
...
In conclusion, some possible reasons for why Thompson attained better yields than the county averages are his usage of Spring tillage, the rotary hoe, and maunure. However, it is important to note that this information comes only from Thompson's land and cannot be generalized to any other farm. Also these graphs do not take into account year-to-year variability due to rainfall, temperature, and other influences.